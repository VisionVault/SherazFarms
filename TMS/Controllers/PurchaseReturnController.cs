...
namespace TMS.Controllers
{
    ...
    public class PurchaseReturnController : Controller
    {
        ...
        private readonly DocType PurchaseRetDT;
        ...
        public PurchaseReturnController(TMSContext context, UserManager<User> usrMgr,
            IHttpContextAccessor http, IWebHostEnvironment env)
        {
            ...
            PurchaseRetDT = context.DocTypes.Where(a => a.Name == "PRV").FirstOrDefault();
        ...
        public JsonResult GetAll(IFormCollection vm)
        {
            JsonResult json = new JsonResult("");
            json.StatusCode = 404;
            if (PurchaseRetDT == null)
            {
                json.Value = "Error! Purchase Return Doc Type not found";
                return json;
            }
            ...
            try
            {
                ...
                && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3)
        ...
        public JsonResult Recall(long id)
        {
            ...
            try
            {
                ...
                var data = (from pl in context.ProductLines
                            .Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3)
        ...
        //reports
        [HttpPost]
        public IActionResult PurchaseRetReport(IFormCollection vm)
        {
            try
            {
                if (!DateTime.TryParse(vm["FromDate"].ToString(), out _))
                ...
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                List<SaleReportVM> data = (from p in context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3
        ...
        {
            ViewBag.DocId = Convert.ToInt64(GetMaxDocId(PurchaseRetDT.Id).Value);
        ...
        public JsonResult Add(List<DocVM> vm)
        {
            ...
            var maxCode = GetMaxDocId(PurchaseRetDT.Id);
            ...
            doc.DocTypeId = PurchaseRetDT.Id;
        ...
        public JsonResult Remove(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
                    foreach (var item in data)
                    {
                ...
                var pl = context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3).ToList();
                if (pl.Count > 0)
                {
        ...
        public JsonResult Delete(long docId)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
                ...
                var pd = context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == PurchaseRetDT.Id && a.TransactionStatusId != 3).ToList();
                if (pd.Count > 0)
                {
