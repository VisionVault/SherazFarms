...
namespace TMS.Controllers
{
    ...
    public class SaleController : Controller
    {
        ...
        private readonly Account SaleAc;
        ...
        public SaleController(TMSContext context, UserManager<User> usrMgr,
            IHttpContextAccessor http, IWebHostEnvironment env)
        {
            ...
            SaleAc = context.Accounts.Where(a => a.Name == "Sale").FirstOrDefault();
        ...
        public JsonResult GetAll(IFormCollection vm)
        {
            ...
            if (SaleDT == null)
            {
                json.Value = "Error! Sale Doc Type not found";
                return json;
            }
            ...
            try
            {
                ...
                var data = (from pl in context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.TransactionDate >= fromDate && a.TransactionDate <= toDate
                                && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3)
                            group new { pl.ActualWeight, pl.Net} by new
                            {
                                pl.DocId,
                                pl.TransactionDate,
                                pl.VehicleNumber,
                            ...
                            select new
                            {
                                ...
                                g.Key.Account,
                                g.Key.PaymentTerm,
                                ActualWeight = g.Sum(a => a.ActualWeight),
                                Amount = g.Sum(a => a.Net),
                            }).ToList();
        ...
        public JsonResult Recall(long id)
        {
            ...
            try
            {
                ...
                var data = (from pl in context.ProductLines
                            .Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3)

                            let bankAc = context.LedgerDetails.Where(b => b.DocId == pl.DocId && b.DocTypeId == SaleDT.Id
        ...
        public IActionResult SaleReport(IFormCollection vm)
        {
            try
            {
                ...
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                List<SaleReportVM> data = (from p in context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3
                                          && a.TransactionDate >= fromDate && a.TransactionDate <= toDate)
                                           ...
                                           select new SaleReportVM
                                           {
                                               ...
                                               Contact = context.AccountDetails.Where(b => b.AccountId == p.AccountId).FirstOrDefault().Contact,
                                               Address = context.AccountDetails.Where(b => b.AccountId == p.AccountId).FirstOrDefault().Address,
                                               ...
                                               LoadedWeight = p.LoadedWeight,
                                               ActualWeight = p.ActualWeight,
                                               Rate = p.Rate,
                                               Amount = p.Amount,
                                               DiscountP = p.DiscountP,
                                               Net = p.Net,
                                               BillDiscount = p.BillDiscount,
                                               Payment = p.Payment
                                           }).ToList();
                data = data.OrderBy(a => a.TransactionDate).ThenBy(a => a.AccountId).ThenBy(a => a.DocId).ToList();
                if (accountId > 0)
                {
        ...
        {
            ViewBag.DocId = Convert.ToInt64(GetMaxDocId(SaleDT.Id).Value);
        ...
        public JsonResult Add(List<DocVM> vm)
        {
            ...
            if (SaleAc == null)
            {
                json.Value = "Error! Sale account not found";
                return json;
            }
            if (DiscAll == null)
            {
            ...
            foreach(var item in vm)
            {
                ...
                double disc = item.Amount - item.Net;
                item.Discount = disc;
            }
            var maxCode = GetMaxDocId(SaleDT.Id);
        ...
        public JsonResult Remove(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
                ...
                var pl = context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3).ToList();
                if (pl.Count > 0)
                {
        ...
        public JsonResult Delete(long docId)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
                ...
                var pd = context.ProductLines.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == SaleDT.Id && a.TransactionStatusId != 3).ToList();
                if (pd.Count > 0)
                {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    if (i == 1)
                    {
                        ...
                        else
                        {
                            ...
                            ld.Narration = "Discount Allowed on invoice " + doc.DocId;
                            ld.NarrationDetailed = "Discount Allowed on invoice " + doc.DocId;
                            ld.Debit = discount;
                            ld.Credit = 0;
                        }
                    }
                    ...
                    if (i == 5)
                    {
                        ...
                        else
                        {
                            if (doc.PaymentTermId == 3)
                            {
                                ...
                                ld.AgainstAccountId = doc.AccountId;
                                ld.Narration = "Cash received through bank on invoice " + doc.DocId;
                                ld.NarrationDetailed = "Cash received through bank on invoice " + doc.DocId;
                            ...
                            else
                            {
                                ...
                                ld.AgainstAccountId = doc.AccountId;
                                ld.Narration = "Cash received on invoice " + doc.DocId;
                                ld.NarrationDetailed = "Cash received on invoice " + doc.DocId;
                                ld.Debit = doc.Payment;
                                ld.Credit = 0;
                            }                            
                        }
                    ...
                    if (i == 6)
                    {
                        ...
                        else
                        {
                            if (doc.PaymentTermId == 3)
                            {
                                ...
                                ld.Narration = "Cash received thorugh bank on invoice " + doc.DocId;
                                ld.NarrationDetailed = "Cash received thorugh bank on invoice " + doc.DocId;
                                ld.Debit = 0;
        ...
        public JsonResult Stock(List<DocVM> vm)
        {
            ...
            try
            {
                ...
                foreach(var item in vm)
                {
                    ...
                    pl.Stock = item.ActualWeight * -1;
                    pl.StockValue = Math.Round(item.ActualWeight * cost, 2) * -1;
