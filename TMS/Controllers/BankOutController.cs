...
{
    [Authorize]
    public class BankOutController : Controller
    {
        private readonly TMSContext context;
        private readonly UserManager<User> usrMgr;
        private readonly DocType BPVDT;

        public BankOutController(TMSContext context, UserManager<User> usrMgr)
        {
            this.context = context;
            this.usrMgr = usrMgr;
            BPVDT = context.DocTypes.Where(a => a.Name == "BPV").FirstOrDefault();
        }
        ...
        public JsonResult GetMaxDocId()
        {
            ...
            try
            {
                ...
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocTypeId == BPVDT.Id).OrderByDescending(a => a.DocId).FirstOrDefault();
        ...
        public JsonResult GetAll(IFormCollection vm)
        {
            ...
            try
            {
                ...
                && a.DocTypeId == BPVDT.Id && a.TransactionStatusId != 3 && a.Debit > 0)
                    .Select(a => new
                    {
                        a.DocId,
                        a.TransactionDate,
        ...
        public JsonResult Recall(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == BPVDT.Id && a.TransactionStatusId != 3 && a.Debit > 0)
                    .Select(a => new
                    {
                        a.DocId,
                        a.TransactionDate,
        ...
        public JsonResult Remove(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == BPVDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
        ...
        public JsonResult Delete(long docId)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == BPVDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
