...
namespace TMS.Controllers
{
    ...
    public class CashOutController : Controller
    {
        ...
        private readonly DocType CPVDT;

        public CashOutController(TMSContext context, UserManager<User> usrMgr)
        {
            this.context = context;
            this.usrMgr = usrMgr;
            CPVDT = context.DocTypes.Where(a => a.Name == "CPV").FirstOrDefault();
        }
        ...
        public JsonResult GetMaxDocId()
        {
            ...
            try
            {
                ...
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocTypeId == CPVDT.Id).OrderByDescending(a => a.DocId).FirstOrDefault();
        ...
        public JsonResult GetAll(IFormCollection vm)
        {
            ...
            try
            {
                ...
                && a.DocTypeId == CPVDT.Id && a.TransactionStatusId != 3 && a.Debit > 0)
                    .Select(a => new
                    {
                        a.DocId,
                        a.TransactionDate,
        ...
        public JsonResult Recall(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == CPVDT.Id && a.TransactionStatusId != 3 && a.Debit > 0)
                    .Select(a => new
                    {
                        a.DocId,
                        a.TransactionDate,
        ...
        public JsonResult Remove(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == CPVDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
        ...
        public JsonResult Delete(long docId)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == CPVDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
