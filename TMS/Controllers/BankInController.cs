...
{
    [Authorize]
    public class BankInController : Controller
    {
        private readonly TMSContext context;
        private readonly UserManager<User> usrMgr;
        private readonly DocType BRVDT;

        public BankInController(TMSContext context, UserManager<User> usrMgr)
        {
            this.context = context;
            this.usrMgr = usrMgr;
            BRVDT = context.DocTypes.Where(a => a.Name == "BRV").FirstOrDefault();
        }
        ...
        public JsonResult GetMaxDocId()
        {
            ...
            try
            {
                ...
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocTypeId == BRVDT.Id).OrderByDescending(a => a.DocId).FirstOrDefault();
        ...
        public JsonResult Search(string param)
        {
            ...
            try
            {
                var code = context.Accounts.Where(a => a.Name.ToLower().Contains(param.ToLower()))
                    .Select(a => new
                    {
                        a.Id,
                        a.Code,
        ...
        public JsonResult GetAll(IFormCollection vm)
        {
            ...
            try
            {
                ...
                && a.DocTypeId == BRVDT.Id && a.TransactionStatusId != 3 && a.Credit > 0)
                    .Select(a => new
                    {
                        a.DocId,
                        a.TransactionDate,
        ...
        public JsonResult Recall(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == BRVDT.Id && a.TransactionStatusId != 3 && a.Credit > 0)
                    .Select(a => new
                    {
                        a.DocId,
                        a.TransactionDate,
                        ...
                        a.AccountId,
                        Account = a.Account.Name,
                        CashAcId = a.AgainstAccountId,
                        CashAc = a.AgainstAccount.Name,
                        a.Narration,
                        Amount = a.Credit
                    }).ToList();
        ...
        public JsonResult Remove(long id)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == id && a.DocTypeId == BRVDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
        ...
        public JsonResult Delete(long docId)
        {
            ...
            {
                var tf = SysAcc.GetTF(context, usrMgr.GetUserId(User));
                var data = context.LedgerDetails.Where(a => a.TradeFirmId == tf.Id && a.DocId == docId && a.DocTypeId == BRVDT.Id && a.TransactionStatusId != 3).ToList();
                if (data.Count > 0)
                {
