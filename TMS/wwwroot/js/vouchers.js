    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length > 0) {
                ...
                if (c == 'CashIn') {
                    newOption($('#CashAcId'), data[0].cashAc, data[0].cashAcId);
                }
                if (c == 'BankIn') {
                    newOption($('#BankAcId'), data[0].cashAc, data[0].cashAcId);
                }
                if (c == 'CashOut') {
                    newOption($('#CashAcId'), data[0].cashAc, data[0].cashAcId);
                }
...
function AR(tBody) {
    var selectData = $('#AccountId').select2('data');
    if (rIndex > -1) {
        var row = tBody.find('tr:eq(' + rIndex + ')');
        row.find('td:eq(0)').text($('#AccountId').val());
        row.find('td:eq(2)').text(selectData[0].name);
        row.find('td:eq(3)').text($('#Narration').val());
        row.find('td:eq(4)').text($('#Amount').val());
        rIndex = -1;
    }
    else {
...
function UR(row) {
    ...
    var narration = row.find('td:eq(3)').text();
    var amount = parseFloat(row.find('td:eq(4)').text());
    newOption($('#AccountId'), name, id);
    ...
    $.each(tBody.find('tr'), function () {
        ...
        if (c == 'CashIn') {
            d = {
                ...
                DebitAcId: parseInt($('#CashAcId').val()),
                CreditAcId: parseInt(row.find('td:eq(0)').text()),
        ...
        if (c == 'BankIn') {
            d = {
                DocId: parseInt($('#DocId').val()),
                TransactionDate: $('#TransactionDate').val(),
                DebitAcId: parseInt($('#BankAcId').val()),
                CreditAcId: parseInt(row.find('td:eq(0)').text()),
                Narration: row.find('td:eq(3)').text(),
                Amount: parseFloat(row.find('td:eq(4)').text()),
            }
        ...
        if (c == 'CashOut') {
            d = {
                DocId: parseInt($('#DocId').val()),
                TransactionDate: $('#TransactionDate').val(),
                CreditAcId: parseInt($('#CashAcId').val()),
        ...
        if (c == 'BankOut') {
            d = {
                DocId: parseInt($('#DocId').val()),
                TransactionDate: $('#TransactionDate').val(),
                CreditAcId: parseInt($('#BankAcId').val()),
                DebitAcId: parseInt(row.find('td:eq(0)').text()),
                Narration: row.find('td:eq(3)').text(),
                Amount: parseFloat(row.find('td:eq(4)').text()),
            }
...
//select2 Eve
$('#CashAcId').on('select2:close', function () {
    $('#AccountId').select2('open');
})
$('#BankAcId').on('select2:close', function () {
    $('#AccountId').select2('open');
})
